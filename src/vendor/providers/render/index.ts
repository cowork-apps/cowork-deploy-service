...
export class RenderServiceHost implements ServiceHost {
  ...
  async deploy(s: ServiceRef, commitSha: string, env: Record<string, string>): Promise<DeployRef> {
    ...
    if (Object.keys(env).length) {
      const current: any[] = await json(`${this.api}/services/${s.id}/env-vars`, { token: this.token }).catch(() => []);
      const have = Object.fromEntries(current.map((e) => [e.envVar?.key, e.envVar?.value]));
      envChanged = Object.entries(env).some(([k, v]) => have[k] !== v) || Object.keys(have).some((k) => !(k in env));
      if (envChanged) await json(`${this.api}/services/${s.id}/env-vars`, { method: "PUT", token: this.token,
    ...
    // so: reuse an in-flight deploy for this commit if there is one, else trigger and re-list. Never reuse one when
    // the env just changed: a deploy that started before the change runs without it.
    const inflight = envChanged ? null : await this.findDeploy(s, commitSha);
