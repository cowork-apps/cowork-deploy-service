...
export function withPersistence(input: DeployInput) {
  ...
  // The sidecar's env must be present when the service is CREATED: Render starts the first build/deploy at creation,
  // and env added afterwards is not seen by that running deploy (sunny's app ran a week without persistence, 2026-09-17).
  return { ...spec, env: { ...spec.env, ...persistenceEnv(input) }, build: `${spec.build} && curl -fsSL ${input.appdata.agentUrl} -o .cowork-persist.py`, start };
}
